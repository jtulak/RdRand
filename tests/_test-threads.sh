...
# Limit the test to run on only one socket throught numactl -i 1
...
for ((THREADS=$MIN ; THREADS<=$MAX ; THREADS++ )); do
  ...
  if [ $FORCE_SOCKET == 0 ]; then
    ...
    CMD="numactl -i 1 '$BIN' '$FILE_STDOUT' -d $DURATION -c $CHUNK -r $REPETITION -t $THREADS -m rdrand_get_bytes_retry"
  fi
